...
# Builds a distributable, Developer ID-signed, notarized superUsage.app and wraps it in a DMG. The app
...
#   SUPERUSAGE_VERSION     human version, e.g. 0.7.0 (CFBundleShortVersionString)
# Optional env:
#   SUPERUSAGE_BUILD       CFBundleVersion (monotonic). Default: git commit count.
...
: "${SUPERUSAGE_VERSION:?set SUPERUSAGE_VERSION, e.g. 0.7.0}"

APP_NAME="superUsage"
BUNDLE_ID="com.weisenjoytech.superusage"
MIN_SYSTEM_VERSION="15.0"
VERSION="$SUPERUSAGE_VERSION"
...
BUILD="${SUPERUSAGE_BUILD:-$(git rev-list --count HEAD)}"
FEED_URL="${FEED_URL:-https://jewel591.github.io/superusage/appcast.xml}"
...
ENTITLEMENTS_TEMPLATE="$ROOT_DIR/script/SuperUsage.release.entitlements.plist"
ENTITLEMENTS="$DIST_DIR/SuperUsage.release.resolved.entitlements.plist"
...
swift build -c release --arch arm64 --arch x86_64 -Xswiftc -g --product superUsage
swift build -c release --arch arm64 --arch x86_64 -Xswiftc -g --product superusage-cli
...
if [ -f "$ROOT_DIR/assets/AppIcon.prebuilt/Assets.car" ]; then
...
elif [ -d "$ROOT_DIR/assets/AppIcon.icon" ]; then
  ...
  echo "WARNING: no superUsage app icon is committed yet; using the system default icon" >&2
fi
...
"$ROOT_DIR/script/render_icloud_entitlements.sh" \
  ...
  "iCloud.com.weisenjoytech.usage.sync"
...
codesign -d --entitlements :- "$APP_BUNDLE" 2>&1 | grep -q "iCloud.com.weisenjoytech.usage.sync" \
