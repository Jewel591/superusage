...
# Builds superUsage, stages a signed .app bundle under dist/, and launches it in place — no install
...
#   - uses the Chengdu Weisen Quwan Technology company-team bundle id and CloudKit container;
...
TARGET_NAME="superUsage"                 # SwiftPM product / binary name
APP_DISPLAY="superUsage"                 # user-facing app name
BUNDLE_ID="${BUNDLE_ID:-com.weisenjoytech.superusage}"
ICLOUD_CONTAINER_ID="iCloud.com.weisenjoytech.usage.sync"
MIN_SYSTEM_VERSION="15.0"
# Development-only placeholders. Release versions are supplied explicitly to release.sh after owner
# approval; this bootstrap script must not imply that superUsage inherited an upstream release number.
...
CLI_BINARY="$APP_HELPERS/superusage"
INFO_PLIST="$APP_CONTENTS/Info.plist"
ENTITLEMENTS="$ROOT_DIR/script/SuperUsage.dev.entitlements.plist"
SIGN_ENTITLEMENTS="$ROOT_DIR/script/SuperUsage.local.entitlements.plist"
...
BUILD_CLI_BINARY="$BUILD_DIR/superusage-cli"
...
# SuperUsage_SuperUsage.bundle, which carries the provider SVGs + model manifest)
# into Contents/Resources, the standard app layout. Bundle.superUsageResources
...
<dict>
  ...
  <true/>
  <key>SuperUsageCloudKitContainerIdentifier</key>
...
if [ -n "${ICLOUD_PROVISIONING_PROFILE:-}" ]; then
  ...
  SIGN_ENTITLEMENTS="$DIST_DIR/SuperUsage.dev.resolved.entitlements.plist"
...
# Never choose the first certificate: this Mac also has a company developer account.
# The default below belongs to Chengdu Weisen Quwan Technology (Team ID C554753V8P).
CODESIGN_IDENTITY="${CODESIGN_IDENTITY:-Apple Development: Lin Liao (9HG68V2UWF)}"
if ! /usr/bin/security find-identity -p codesigning -v 2>/dev/null \
  | /usr/bin/grep -Fq "$CODESIGN_IDENTITY"; then
  echo "required C554753V8P signing identity is unavailable: $CODESIGN_IDENTITY" >&2
  exit 1
fi
