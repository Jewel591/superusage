...
final class AppBundleLocatorTests: XCTestCase {
    func testFindsContainingAppThroughHelperSymlink() throws {
        ...
        let plist: [String: Any] = [
            "CFBundleIdentifier": "com.example.superusage",
            "CFBundleShortVersionString": "1.2.3"
        ]
        let plistData = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try plistData.write(to: contents.appendingPathComponent("Info.plist"))
        let helper = helpers.appendingPathComponent("superusage")
        try Data().write(to: helper)
        let symlink = bin.appendingPathComponent("superusage")
        ...
        XCTAssertEqual(located.bundleIdentifier, "com.example.superusage")
        XCTAssertEqual(located.version, "1.2.3")
    }

    func testEnvironmentCanSelectDefaultsSuiteForDevelopment() {
        let located = AppBundleLocator.locate(
            executableURL: URL(fileURLWithPath: "/tmp/superusage"),
            environment: ["SUPERUSAGE_DEFAULTS_SUITE": "com.example.dev"]
        )
