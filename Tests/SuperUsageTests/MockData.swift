...
/// Test fixtures: sample providers and the widgets they register (mirroring SuperUsage's real
/// metrics and units). Lives in the test target only — the app builds its registry from the
/// live `ProviderRuntime`s.
enum MockData {
    // MARK: - Providers (brand colors copied from SuperUsage plugin.json)
