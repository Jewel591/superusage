...
final class IncrementalJSONLScannerTests: XCTestCase {
    ...
    func testLimitsConcurrentParsesAndKeepsFileOrder() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageScannerTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testUnreadableFileWarnsOnceUntilItRecovers() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageScannerWarnings-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func testScanningAnotherBatchDoesNotForgetAnUnreadableFile() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageScannerWarningBatches-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func testJsonlFilesFollowsSymlinkedRoot() throws {
        ...
        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageScannerSymlink-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMissingFileDoesNotWarn() async {
        ...
        let file = JSONLScanning.DiscoveredFile(
            path: "/tmp/superusage-missing-\(UUID().uuidString).jsonl",
            size: 0,
            mtime: Date()
        )
    ...
    private func makeDirectory(_ suffix: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageScanner\(suffix)-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory
    }
