...
final class LogFileTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("superUsageTests.LogFile.\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func testResolvedPathEndsWithExpectedSuffix() {
        XCTAssertTrue(
            LogFile.url.path.hasSuffix("Logs/superUsage/superUsage.log"),
    ...
    func testAppendCreatesFileAndWritesLine() throws {
        let log = LogFile(directory: tempDir, fileName: "superUsage.log")
        log.open()
        log.append("2026-01-01T00:00:00Z [INFO] [config] hello")

        let fileURL = tempDir.appendingPathComponent("superUsage.log")
    ...
    func testRotationCreatesBackupWhenCapExceeded() throws {
        ...
        let cap = 200
        let log = LogFile(directory: tempDir, fileName: "superUsage.log", maxBytes: cap)
        log.open()
        ...
        let mainURL = tempDir.appendingPathComponent("superUsage.log")
        let archiveURL = tempDir.appendingPathComponent("superUsage.1.log")
    ...
    func testRotationKeepsOnlyOneArchive() throws {
        ...
        let archiveURL = tempDir.appendingPathComponent("superUsage.1.log")
        let secondArchiveURL = tempDir.appendingPathComponent("superUsage.2.log")
