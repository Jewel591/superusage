...
final class CodexAuthStoreTests: XCTestCase {
    ...
    func testNoExpClaimFallsBackToStaleLastRefresh() {
        ...
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        let store = CodexAuthStore(now: { now })
        let nineDaysAgo = SuperUsageISO8601.string(from: now.addingTimeInterval(-9 * 24 * 60 * 60))
        let auth = CodexAuth(
...
final class CodexUsageMapperTests: XCTestCase {
    ...
    func testAppendsTokenUsageLines() {
        ...
        let now = makeDate("2026-02-20T08:00:00.000Z")
        let today = DailyUsageAccumulator.dayKey(from: now)
        let earlier = Calendar.current.date(byAdding: .day, value: -19, to: now)!
        let earlierKey = DailyUsageAccumulator.dayKey(from: earlier)
        let usage = DailyUsageSeries(daily: [
            DailyUsageEntry(date: today, totalTokens: 150, costUSD: 0.75),
            DailyUsageEntry(date: earlierKey, totalTokens: 300, costUSD: 1.0)
        ])
    ...
    func testZeroUsageLeavesTilesUnbacked() {
        ...
        SpendTileMapper.appendTokenUsage(
            DailyUsageSeries(daily: [
                DailyUsageEntry(
                    date: DailyUsageAccumulator.dayKey(from: yesterday),
                    totalTokens: 0,
                    costUSD: nil
                )
            ]),
    ...
    func testDedicatedEndpointSuppliesCountAndSortedExpiries() throws {
        ...
        let mapped = try CodexUsageMapper.mapUsageResponse(
            ...
            now: SuperUsageISO8601.date(from: "2026-02-20T16:00:00.000Z")!
        )
        ...
        XCTAssertEqual(expiriesAt, [
            SuperUsageISO8601.date(from: "2026-02-20T17:30:00.000Z")!,
            SuperUsageISO8601.date(from: "2026-02-20T19:00:00.000Z")!
        ])
    }
    ...
    private func makeDate(_ value: String) -> Date {
        SuperUsageISO8601.date(from: value)!
    }
}
