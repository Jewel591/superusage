...
final class AntigravityProviderTests: XCTestCase {
    ...
    func testGoogleRefreshFormEncodesReservedCharactersInRequestBody() async throws {
        ...
        let credentials = GoogleOAuthClientCredentials(
        ...
        XCTAssertEqual(
            ...
            "client_id=test%20client%26id" +
                "&client_secret=test%20secret%3D%2B" +
                "&refresh_token=refresh%20token%26%3D%2B%2F%3F%25" +
                "&grant_type=refresh_token"
        )
    }

    func testGoogleRefreshWithoutPublicSourceCredentialsDoesNotSendARequest() async {
        let http = FakeHTTPClient(response: HTTPResponse(statusCode: 500, headers: [:], body: Data()))
        let client = AntigravityUsageClient(
            lsHTTP: http,
            http: http,
            googleOAuthCredentials: nil
        )

        let result = await client.refreshGoogleToken("refresh-token")

        guard case .unavailable = result else {
            return XCTFail("missing OAuth client configuration must be unavailable")
        }
    ...
    func testExtractTokenFromGoKeyringWrappedJSON() {
        ...
        XCTAssertEqual(token?.expiry, SuperUsageISO8601.date(from: "2099-01-01T00:00:00Z"))
    }
