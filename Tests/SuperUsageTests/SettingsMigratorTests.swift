...
final class SettingsMigratorTests: XCTestCase {
    ...
    func testLegacyInstallMigratesFromZeroAndKeepsSettings() {
        ...
        defer { defaults.removePersistentDomain(forName: domain) }
        defaults.set("custom", forKey: "superusage.layout.v1")  // pre-existing settings, no schema version
        ...
        XCTAssertEqual(defaults.string(forKey: "superusage.layout.v1"), "custom", "existing settings preserved")
    }
    ...
    func testMigrateNeverWipesExistingSettings() {
        ...
        defaults.set("custom", forKey: "superusage.layout.v1")
        defaults.set(720.0, forKey: "superusage.panelHeight")
        ...
        XCTAssertEqual(defaults.string(forKey: "superusage.layout.v1"), "custom")
        XCTAssertEqual(defaults.double(forKey: "superusage.panelHeight"), 720.0)
    }
    ...
    func testV2ConvertsLegacyDisabledListToEnabledList() {
        ...
        defaults.set(["devin", "grok"], forKey: "superusage.disabledProviders.v1")

        let result = SettingsMigrator.migrate(defaults: defaults, domainName: domain)

        XCTAssertEqual(result, SettingsSchema.current)
        let enabled = Set(defaults.stringArray(forKey: "superusage.enabledProviders.v1") ?? [])
        XCTAssertEqual(enabled, Set(SettingsSchema.v2ProviderIDs).subtracting(["devin", "grok"]))
        XCTAssertNil(defaults.stringArray(forKey: "superusage.disabledProviders.v1"), "legacy key removed")
        XCTAssertEqual(
            Set(defaults.stringArray(forKey: "superusage.knownProviders.v1") ?? []),
    ...
    func testV2ConvertsAllOnLegacyInstall() {
        ...
        defer { defaults.removePersistentDomain(forName: domain) }
        defaults.set(1, forKey: SettingsMigrator.schemaVersionKey)
        defaults.set("custom", forKey: "superusage.layout.v1")  // some settings, so not a fresh install

        SettingsMigrator.migrate(defaults: defaults, domainName: domain)

        XCTAssertEqual(
            Set(defaults.stringArray(forKey: "superusage.enabledProviders.v1") ?? []),
            Set(SettingsSchema.v2ProviderIDs)
        )
    }
    ...
    func testV2LeavesExistingEnabledListAloneAndSeedsKnownSet() {
        ...
        defer { defaults.removePersistentDomain(forName: domain) }
        defaults.set(1, forKey: SettingsMigrator.schemaVersionKey)
        defaults.set(["claude", "cursor"], forKey: "superusage.enabledProviders.v1")
    ...
    func testV2IsIdempotent() {
        ...
        defer { defaults.removePersistentDomain(forName: domain) }
        defaults.set(1, forKey: SettingsMigrator.schemaVersionKey)
        defaults.set(["codex"], forKey: "superusage.disabledProviders.v1")

        SettingsMigrator.migrate(defaults: defaults, domainName: domain)
        let enabledAfterFirst = defaults.stringArray(forKey: "superusage.enabledProviders.v1")
        defaults.set(1, forKey: SettingsMigrator.schemaVersionKey)  // simulate an interrupted upgrade
        SettingsMigrator.migrate(defaults: defaults, domainName: domain)

        XCTAssertEqual(defaults.stringArray(forKey: "superusage.enabledProviders.v1"), enabledAfterFirst)
    }
    ...
    private func makeDefaults(_ name: String) -> (UserDefaults, String) {
        let suite = "superUsageTests.SettingsMigrator.\(name).\(UUID().uuidString)"
