...
struct SuperUsageCLI {
    static func main() async {
        do {
            ...
            if arguments.showVersion {
                print(app.version.map { "superusage \($0)" } ?? "superusage (development build)")
                return
            }
            ...
            fail("\(message)\nRun 'superusage --help' for usage.", code: 2)
        } catch CLIError.appDefaultsUnavailable {
            fail("Could not open the superUsage settings domain.", code: 4)
    ...
    private static func writeError(_ message: String) {
        FileHandle.standardError.write(Data("superusage: \(message)\n".utf8))
    }
    ...
    Usage: superusage [provider] [--force]

    Read limits through SuperUsage's shared five-minute cache and exit. Output is always JSON.
