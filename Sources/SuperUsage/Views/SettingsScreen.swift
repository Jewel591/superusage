...
struct SettingsScreen: View {
    ...
    /// macOS notification authorization for SuperUsage, surfaced in the Notifications section so a
    ...
    private var content: some View {
        ...
        return VStack(alignment: .leading, spacing: density.sectionSpacing) {
            section("General") {
                ...
                row("Global Shortcut") {
                    ShortcutRecorderField(name: .togglePopover)
                        .hoverTooltip("Open superUsage from anywhere")
                }
            }
            AppleDeviceSyncSettingsSection(sync: container.appleDeviceSync)
            section("Appearance") {
                row("Icon Style") {
            ...
            section("Privacy") {
                ...
                Text("While your screen is shared or recorded, the menu bar shows “superUsage” instead of your usage.")
                    .font(.caption)
                ...
                Text("Shares anonymous usage counts and error types to help improve superUsage. No account details, credentials, or usage values are sent.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 12)
                    .padding(.bottom, 8)
                ...
                if needsAttention {
                    Image(systemName: "exclamationmark.triangle")
                        ...
                        .hoverTooltip(notificationsAuth == .denied
                            ? "Notifications are turned off for superUsage. Enable them in System Settings."
                            : "superUsage needs permission to send alerts.")
                }
            }
    ...
    private var commandLineSection: some View {
        section("Command Line") {
            ...
            Text("Adds a global `superusage` command agents can use to monitor limits.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .padding(.horizontal, 12)
                .padding(.bottom, 8)
            ...
            if commandLineTool.status == .conflict {
                inlineNotice("\(commandLineTool.destinationPath) already exists and wasn't installed by superUsage.")
    ...
    /// (`~/Library/Logs/superUsage/superUsage.log`); raising the level here applies live (no restart) and
