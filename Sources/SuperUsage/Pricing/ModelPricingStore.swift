...
/// SuperUsage pricing supplement on gh-pages). `current()` never blocks on the network — it serves
...
actor ModelPricingStore {
    ...
    static let defaultSourceURLs: [SourceID: URL] = [
        ...
        .supplement: URL(string: "https://jewel591.github.io/superusage/pricing_supplement.json")!
    ]

    private static var defaultCacheDirectory: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("superUsage/pricing", isDirectory: true)
    }

    private static func bundledResourceData(_ resourceName: String) -> Data? {
        guard let url = Bundle.superUsageResources.url(forResource: resourceName, withExtension: "json") else {
            return nil
        }
