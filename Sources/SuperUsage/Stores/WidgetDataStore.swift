...
final class WidgetDataStore {
    ...
    /// Last-good, locally produced snapshots that are safe to publish to read-only Apple clients.
    /// Disabled providers stay cached locally but are excluded from the exported snapshot.
    func appleDeviceExportSnapshots() -> [ProviderSnapshot] {
        localSnapshots.values
            .filter { isProviderEnabled($0.providerID) }
