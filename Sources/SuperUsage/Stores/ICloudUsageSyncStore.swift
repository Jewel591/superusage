...
struct KeychainICloudDeviceIDStore: ICloudDeviceIDStoring {
    ...
    init(
        ...
        bundleIdentifier: String = Bundle.main.bundleIdentifier ?? "com.weisenjoytech.superusage"
    ) {
...
/// Coordinated access to the app-private data area of SuperUsage's iCloud Documents container.
...
final class ICloudUsageSyncStore {
    private static let enabledKey = "superusage.icloudSync.enabled.v1"
    private static let deviceIDKey = "superusage.icloudSync.deviceID.v1"
    ...
    private static func resolveDeviceID(
        ...
        do {
            ...
            let id = saved ?? UUID().uuidString.lowercased()
            defaults.set(id, forKey: deviceIDKey)
            let message = "superUsage couldn’t save this Mac’s sync identity in Keychain. "
