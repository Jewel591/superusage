...
final class AppNotifications: NSObject, UNUserNotificationCenterDelegate {
    ...
    /// Open System Settings → Notifications so the user can re-enable alerts for SuperUsage after a
    ...
    func post(idPrefix: String, title: String, subtitle: String, body: String, soundEnabled: Bool = true) async -> Bool {
        ...
        content.body = body
        // Group all SuperUsage alerts into one stacked thread so simultaneous alerts (e.g. a metric
        // that fires two milestones at once) collapse into a single banner with a "N more" summary
        // instead of separate banners.
        content.threadIdentifier = "superusage"
        if soundEnabled { content.sound = .default }
        let id = "superusage-\(idPrefix)-\(UUID().uuidString)"
    ...
    nonisolated func userNotificationCenter(
        ...
        guard response.actionIdentifier == UNNotificationDefaultActionIdentifier,
              response.notification.request.content.threadIdentifier == "superusage"
        else {
            completionHandler()
            return
        }
