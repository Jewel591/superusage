...
/// current file becomes `superUsage.1.log` and a fresh `superUsage.log` opens — bounding disk to ~20 MB
...
final class LogFile: @unchecked Sendable {
    ...
    /// `~/Library/Logs/superUsage/superUsage.log` via `FileManager`, never hardcoded from `$HOME`; the
    /// `Logs/superUsage` subfolder is a literal (not bundle-id-keyed), so the dev and release builds
    /// agree on the same file — acceptable since they are separate builds.
    static let shared = LogFile(directory: defaultDirectory(), fileName: "superUsage.log")
    ...
    private let fallbackLogger = Logger(subsystem: "superUsage", category: "logfile")
    ...
    static func defaultDirectory() -> URL {
        ...
        return library.appendingPathComponent("Logs/superUsage", isDirectory: true)
    }
