...
enum AppLog {
    ...
    /// gate actually writes; production uses the shared `~/Library/Logs/superUsage/superUsage.log` appender.
    ...
    static func bootstrap() {
        ...
        info(LogTag.config.rawValue,
             "superUsage v\(AppInfo.version) starting (level=\(level.rawValue), log=\(displayPath))")
    }
    ...
    private static func emit(_ level: Level, _ tag: String, _ message: () -> String) {
        ...
        let timestamp = SuperUsageISO8601.string(from: Date())
    ...
    private static func logger(for tag: String) -> Logger {
        ...
        let logger = Logger(subsystem: "superUsage", category: tag)
        loggers[tag] = logger
        return logger
    }
}
