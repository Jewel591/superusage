...
extension Bundle {
    /// The bundle carrying SuperUsage's copied resources (provider SVGs, pricing supplement + snapshots).
    ///
    ...
    static let superUsageResources: Bundle = {
        let bundleNames = ["superUsage_SuperUsage.bundle", "SuperUsage_SuperUsage.bundle"]
        ...
        for case let base? in searchBases {
            for bundleName in bundleNames {
                ...
                if isValidSuperUsageResourceBundle(bundle) {
                    return bundle
                }
                // `Bundle(url:)` succeeds for ANY directory at the path, not only a real resources
                // bundle. Skip stale or empty candidates and keep searching.
...
/// Sentinel check: a valid SuperUsage resource bundle carries the bundled pricing supplement at its
...
private func isValidSuperUsageResourceBundle(_ bundle: Bundle) -> Bool {
