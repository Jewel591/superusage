...
final class CommandLineToolInstaller {
    ...
    init(
        sourcePath: String = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/superusage").path,
        destinationPath: String = "/usr/local/bin/superusage",
    ...
    func install() {
        ...
        guard status != .conflict else {
            errorMessage = "\(destinationPath) already exists and wasn't installed by superUsage."
            return
        }
        guard fileManager.isExecutableFile(atPath: sourcePath) else {
            errorMessage = "The bundled terminal helper couldn't be found. Reinstall superUsage and try again."
            return
        }
