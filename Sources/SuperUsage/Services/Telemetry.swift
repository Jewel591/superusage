...
/// an `SUPERUSAGE_POSTHOG_TOKEN` environment override is supported for local testing without editing
...
enum TelemetryConfig {
    ...
    /// `SUPERUSAGE_POSTHOG_TOKEN` at runtime for local testing.
    private static let bakedToken = "phc_vGEqXEpQNwViyKnMNWvmKWpv8XxMT3yaeYi6gfidr4nf"

    static var token: String {
        let env = ProcessInfo.processInfo.environment["SUPERUSAGE_POSTHOG_TOKEN"]?
