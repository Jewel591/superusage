...
enum CodexUsageMapper {
    ...
    /// Codex CLI/plugin (so SuperUsage's dollar agrees with Codex's own), which keeps the two values
    ...
    private static func parseExpiry(_ value: Any?) -> Date? {
        if let string = value as? String, let date = SuperUsageISO8601.date(from: string) {
            return date
        }
