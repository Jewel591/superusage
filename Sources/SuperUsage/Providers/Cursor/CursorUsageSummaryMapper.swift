...
enum CursorUsageSummaryMapper {
    static func hasUsableSummaryPayload(_ summary: [String: Any]) -> Bool {
        let start = (summary["billingCycleStart"] as? String).flatMap(SuperUsageISO8601.date(from:))
        let end = (summary["billingCycleEnd"] as? String).flatMap(SuperUsageISO8601.date(from:))
    ...
    static func hasUsableRequestPayload(_ usage: [String: Any]) -> Bool {
        ...
        return (usage["startOfMonth"] as? String).flatMap(SuperUsageISO8601.date(from:)) != nil
    }
    ...
    private static func billingCycle(
        ...
        let requestStart = (requestUsage?["startOfMonth"] as? String).flatMap(SuperUsageISO8601.date(from:))
        return BillingCycle(
