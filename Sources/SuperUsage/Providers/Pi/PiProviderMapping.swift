...
/// Maps a pi session log's `provider` field to the SuperUsage provider whose card its usage belongs on.
...
/// card of its own.
///
/// Only providers SuperUsage already has a card for are listed. Pi providers with no SuperUsage
/// equivalent are intentionally absent and left for future work:
/// - `nvidia-nim` — no SuperUsage card.
///
...
enum PiProviderMapping {
    /// pi `provider` value → SuperUsage `Provider.id`.
    ...
    /// The SuperUsage card id for a pi provider, or nil when pi used a provider SuperUsage doesn't track.
