...
/// Builds a per-day token/cost series from pi's session logs for one SuperUsage card, so usage that
...
actor PiUsageScanner {
    ...
    /// Parse every mapped assistant usage line of one session file. Lines for pi providers SuperUsage
    ...
    static func parseLine(_ data: Data) -> Entry? {
        guard let object = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any],
              ...
              let timestamp = SuperUsageISO8601.date(from: timestampRaw),
