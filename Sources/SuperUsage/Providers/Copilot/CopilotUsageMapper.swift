...
enum CopilotUsageMapper {
    ...
    /// fractional seconds), handled by the shared `SuperUsageISO8601` normalizer; free tier sends a bare
    ...
    private static func parseResetDate(_ value: Any?) -> Date? {
        ...
        if let date = SuperUsageISO8601.date(from: raw) { return date }
    ...
    /// formatter is never mutated after creation (same pattern as `CursorUsageCSV`/`SuperUsageISO8601`).
