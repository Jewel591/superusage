...
struct AntigravityAuthStore: Sendable {
    ...
    static let cachePath = "~/Library/Application Support/superUsage/antigravity/auth.json"
    ...
    /// Remove only SuperUsage's derived token; Antigravity's Keychain entry is never modified.
    ...
    static func tokenFromObject(_ object: [String: Any]) -> AntigravityKeychainToken? {
        ...
        let expiry = firstString(source, ["expiry", "expires_at", "expiresAt"]).flatMap { SuperUsageISO8601.date(from: $0) }
