...
struct GoogleOAuthClientCredentials: Sendable {
    static let clientIDEnvironmentKey = "SUPERUSAGE_ANTIGRAVITY_GOOGLE_CLIENT_ID"
    static let clientSecretEnvironmentKey = "SUPERUSAGE_ANTIGRAVITY_GOOGLE_CLIENT_SECRET"

    let clientID: String
    let clientSecret: String

    static func from(environment: EnvironmentReading) -> Self? {
        guard let clientID = environment.value(for: clientIDEnvironmentKey)?.nilIfEmpty,
              let clientSecret = environment.value(for: clientSecretEnvironmentKey)?.nilIfEmpty
        else {
            return nil
        }
...
struct AntigravityUsageClient: Sendable {
    ...
    var http: HTTPClient
    var googleOAuthCredentials: GoogleOAuthClientCredentials?

    init(
        lsHTTP: HTTPClient = URLSessionHTTPClient(allowsInsecureLoopback: true),
        http: HTTPClient = URLSessionHTTPClient(),
        googleOAuthCredentials: GoogleOAuthClientCredentials? = GoogleOAuthClientCredentials.from(
    ...
    func refreshGoogleToken(_ refreshToken: String) async -> TokenRefreshOutcome {
        guard let url = URL(string: Self.googleOAuthURL),
              let googleOAuthCredentials
        else {
            return .unavailable
        }
        let form = [
            "client_id=\(googleOAuthCredentials.clientID.urlFormEncoded)",
            "client_secret=\(googleOAuthCredentials.clientSecret.urlFormEncoded)",
