...
- The one-shot `superusage` command reuses this same persisted cache for five minutes, refreshes missing or stale entries without starting the app, and exits. `superusage --force` runs the same forced provider refresh as ⌘R regardless of cache age.
...
- Every successful refresh also records one point per metric that has a limit into the local
  ...
  either — that's what leaves an honest gap in the trend instead of repeating the last good value. A
  refresh driven by the `superusage` command records into that same history, so it makes no difference
  to the chart whether the app or the CLI did the asking.
...
`~/Library/Application Support/SuperUsage/log-scan-cache/`. It stores parsed usage events before SuperUsage
...
contribution from the local spend total.
