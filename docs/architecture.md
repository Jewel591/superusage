...
A high-level map of how SuperUsage is put together, for people working on the code. For what the app
...
The `superusage` executable imports the same module. A normal invocation reads `ProviderSnapshotCache`
...
1. **Auth store** — reads credentials that already exist on the machine (config files, keychain). SuperUsage
...
macOS menu-bar apps live in an `NSStatusItem`. SuperUsage shows its content in a custom, key-capable
...
SuperUsage runs on macOS 15 (Sequoia) and later. It is built against the latest SDK and back-deploys:
