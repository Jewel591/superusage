...
**Sync to Apple Devices** is off by default. Turning it on publishes normalized superUsage snapshots
from this Mac to the user's private CloudKit database. Cursornow on iPhone, iPad, and Apple Watch reads
those snapshots; it cannot modify them or access provider credentials and raw logs. Settings reports
the last successful publication time and any CloudKit error. See [CloudKit Sync](icloud-sync.md).
...
SuperUsage can alert you with a macOS notification when a metric runs low or its pace gets worse, so you don't have to keep the popover open to catch a quota creeping toward its limit. Alerts work while the app runs in the menu bar, even with the popover closed.
...
| Copy Log Path | button | Copies the log file path (`~/Library/Logs/superUsage/superUsage.log`) to the clipboard. |
