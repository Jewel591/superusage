...
- `ProviderAccountsStore` (`superusage.providerAccounts.v1`): account records with id, family,
...
- Live: `script/build_and_run.sh`, then `~/Library/Logs/SuperUsage/SuperUsage.log` — discovery trail,
...
- CLI/API: `superusage claude` (alias) and `superusage claude@<hash>` (direct);
