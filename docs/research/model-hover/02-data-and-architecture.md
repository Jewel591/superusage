...
> [`SpendTileMapper.swift`](../../../Sources/SuperUsage/Providers/SpendTileMapper.swift),
> [`HoverPopoverState.swift`](../../../Sources/SuperUsage/Views/HoverPopoverState.swift), and
> [`ModelUsageDetail.swift`](../../../Sources/SuperUsage/Views/ModelUsageDetail.swift) for the current
...
Shared spend tiles are produced in `Sources/SuperUsage/Providers/SpendTileMapper.swift`.
...
It consumes `DailyUsageSeries` from `Sources/SuperUsage/Models/DailyUsageSeries.swift`. That type is intentionally provider-neutral and contains only:
...
The shared pricing engine is in `Sources/SuperUsage/Pricing/`:
...
- `CursorUsageCSVRow` in `Sources/SuperUsage/Providers/Cursor/CursorUsageCSV.swift` carries:
  - `date: Date`
  - `model: String`
  - `maxMode: Bool`
...
- `ClaudeLogUsageScanner.Entry` in `Sources/SuperUsage/Providers/Claude/ClaudeLogUsageScanner.swift` carries:
  - `timestamp: Date`
  - `tokens: TokenBreakdown`
  - `costUSD: Double?`
  - `model: String?`
...
- `CodexLogUsageScanner.Event` in `Sources/SuperUsage/Providers/Codex/CodexLogUsageScanner.swift` carries:
  - `timestamp: Date`
  - `model: String`
  - `input: Int`
  - `cached: Int`
...
- `WidgetDescriptor.spendTiles(provider:)` in `Sources/SuperUsage/Models/WidgetDescriptor+Factories.swift` declares the three spend descriptors:
  - `<provider>.today`
  - `<provider>.yesterday`
  - `<provider>.last30`
...
- Implemented in `Sources/SuperUsage/Views/HoverTooltip.swift`.
...
- `UsageSparkline` in `Sources/SuperUsage/Views/UsageSparkline.swift` attaches hover only to the bar strip, not the whole row title.
- It uses `TrendHoverState` from `Sources/SuperUsage/Views/UsageTrendDetail.swift`:
  - 400ms reveal dwell
...
- Tests in `Tests/SuperUsageTests/UsageTrendTests.swift` cover the open/close/quick-pass behavior.
