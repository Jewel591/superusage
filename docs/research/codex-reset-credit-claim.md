...
SuperUsage already lists these credits (the "Resets" surface on the Codex provider); this
...
enterprise/alternative base URLs; SuperUsage uses the ChatGPT style.

Headers on every call (identical to what SuperUsage's Codex usage client already sends):
...
### List (already implemented in SuperUsage)
...
## Implementation notes for SuperUsage (when we build it)

- The claim is a single POST on infrastructure SuperUsage already talks to; auth, headers,
