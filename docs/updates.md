...
SuperUsage keeps itself up to date using [Sparkle](https://sparkle-project.org), the standard update
framework for Mac apps. Updates are downloaded from SuperUsage's own release feed and verified before
...
- **Manual check.** Open **Settings → Updates** and click **Check for Updates…** at any time.
  For both manual checks and banner installs, SuperUsage brings itself to the foreground before opening
  Sparkle so the update window doesn't get buried behind another app. Because SuperUsage normally lives
...
Update builds are published on SuperUsage's GitHub releases, and the list of available versions (the
"appcast") is served from `https://jewel591.github.io/superusage/appcast.xml`. Each download is
signed two ways — Apple notarization plus SuperUsage's own signature — and the app refuses anything that
