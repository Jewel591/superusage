...
superUsage never asks for a token — it reads what Antigravity already has:

- **Antigravity running** — superUsage talks to the app's local language server (the richest source, and where the plan name comes from).
- **App closed** — it falls back to the OAuth token Antigravity / `agy` store in your macOS Keychain and queries Google's Cloud Code API. An expired token can be refreshed when the public build has the runtime configuration described below (superUsage never writes back to Antigravity's own keychain item). Its short-lived cache is reused only while the same Keychain login is present and readable.

The public source tree does not embed Antigravity's Google OAuth client credentials. Refreshing an
expired Keychain token while Antigravity is closed therefore requires
`SUPERUSAGE_ANTIGRAVITY_GOOGLE_CLIENT_ID` and
`SUPERUSAGE_ANTIGRAVITY_GOOGLE_CLIENT_SECRET` in the app's launch environment. Without them, the
running Antigravity language server remains the preferred source and expired-token refresh reports a
temporary unavailability rather than sending incomplete OAuth credentials.
...
- **"Couldn't read Antigravity credentials…"** — unlock Keychain or sign in to Antigravity again. SuperUsage will not use its cached access token until the current login can be verified.
