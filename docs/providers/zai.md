...
When Z.ai reports your plan name, SuperUsage shows it beside the provider name.

## Where credentials come from

Z.ai has no companion CLI/app that SuperUsage can reuse a credential from, so you supply an API key.
SuperUsage reads it from the first place it finds one, in this order:

1. `~/.config/superusage/zai.json` — `{"apiKey":"…"}` (the file Settings writes to)
...
2. Add the key to SuperUsage via **Settings → API Keys**, **or** export it:
