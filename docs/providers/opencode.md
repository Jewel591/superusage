...
When you have the Go subscription, SuperUsage shows "Go" beside the provider name.
...
word. (When OpenCode ships an official usage API, SuperUsage can switch to authoritative numbers without any
...
Use OpenCode as usual. SuperUsage reads OpenCode's local data directory
...
The Go caps SuperUsage draws against are the published plan limits: **$12 per rolling 5 hours**, **$30 per
...
- **Everything shows "No data"** — SuperUsage needs OpenCode's local database at
...
SuperUsage reads the assistant-message `cost` and token fields from every `opencode*.db` in the data
