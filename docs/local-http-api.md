...
superUsage exposes a read-only HTTP API on the loopback interface so other local apps can consume the same usage data shown in the menu bar.
...
and the exact format printed by the `superusage` CLI.
...
Both routes read the same rendered local provider snapshots as the dashboard. Apple-device CloudKit
publication does not change the local API response; `/v1/usage` returns the old UI-oriented shape,
while `/v1/limits` projects the data into stable resource IDs and raw scalar values.
