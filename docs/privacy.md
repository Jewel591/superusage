...
superUsage can share **anonymous** usage data to help us understand how the app is used and catch problems. It is on by default and you can turn it off any time in **Settings → Privacy → Share Anonymous Usage**.

## What is shared

When sharing is on, superUsage sends two kinds of small daily summaries: one app-use event per day and,
...
- **Crash reports** — if superUsage crashes, it saves a report and sends it the next time you open the app: the technical stack trace (which parts of *superUsage's own code* were running when it crashed) plus the app and macOS version. This contains no account details, credentials, or usage values — just where in the app the crash happened.
...
superUsage primarily reads credentials that provider tools already keep on your Mac. When it writes a
...
Claude Desktop access is strictly read-only. superUsage may ask macOS for permission to use the
...
Besides the provider API calls the vendor's own tools would make, superUsage fetches public [model price lists](pricing.md) about once an hour (from `raw.githubusercontent.com`, `models.dev`, and this project's GitHub Pages). These are plain downloads of public data — they carry no usage, log, or account information, and they run regardless of the Share Anonymous Usage setting. The spend tiles are computed from local CLI logs entirely on your Mac; no log data ever leaves it.

To avoid re-reading unchanged Claude, Codex, and pi logs after every relaunch, superUsage keeps their
parsed usage events in `~/Library/Application Support/superUsage/log-scan-cache/`. These records contain
...
identity caches that have not been used for 35 days are removed. superUsage's pricing engine runs after
...
`~/Library/Application Support/<bundle id>/superUsageQuotaHistory.sqlite`, holds only metric ids,
timestamps, and the used/limit numbers already shown on the dashboard (no credentials, provider
responses, or log contents), and is never sent to PostHog, a provider, or iCloud. Entries older than 35
days are deleted automatically.

If you explicitly turn on [CloudKit Sync](icloud-sync.md), superUsage writes normalized provider
snapshots to your private CloudKit database so Cursornow can display them on your Apple devices.
Credentials, provider responses, and raw logs are never written there. This is separate from anonymous
usage sharing: Apple-device sync defaults off and uses your iCloud account, while the analytics toggle
controls PostHog events.

## How it works

- Data is fully anonymous: superUsage never identifies you to the analytics service and creates no user profile.
