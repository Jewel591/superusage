...
SuperUsage ships a one-shot `superusage` command for agents and scripts. It prints the documented
...
```sh
superusage                 # every enabled provider, refreshing stale cache entries
superusage codex           # one provider, refreshing when its cache is stale
superusage codex --force   # refresh through the shared provider engine, cache, print, exit
```
...
In SuperUsage, open **Settings → Command Line** and click **Install…**. After the standard macOS
administrator prompt, `superusage` is available globally in new terminal sessions. The installed symlink
points to the signed helper inside SuperUsage, so in-place app updates also update the command.
